<template>
    <BaseDialog v-if="inputIsInvalid" title="Invalid Input" @close="confirmErr">
        <template #section>
            <p>You must enter somthing</p>
            <p>Pls write some info</p>
        </template>
        <template #actions>
            <base-button @click="confirmErr">
                Okey
            </base-button>
        </template>
    </BaseDialog>
  ...
  export default {
    ...
    methods: {
      submitData() {
        const enteredTitle = this.$refs.titleInput.value; // Fixed typo
